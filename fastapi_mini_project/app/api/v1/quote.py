...
from app.schemas.quote import QuoteResponse, BookmarkCreate, BookmarkResponse
...
async def add_favorite_quote(
    request: BookmarkCreate, # BookmarkCreateRequest -> BookmarkCreate
    current_user = Depends(get_current_user)
):
    return await quote_service.add_bookmark(current_user.id, request.quote_id)

# 3. 내 북마크 목록 조회 (명언 내용 리스트 반환)
