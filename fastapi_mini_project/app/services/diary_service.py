...
# Read (단일)
async def get_diary(diary_id: int, user_id: int):
    diary = await diary_repository.get_by_id(diary_id, user_id)
    if not diary:
        raise HTTPException(
...
async def get_diary_list(
    ...
    if size < 5 or size > 20:
        raise HTTPException(status_code=400, detail="목록은 5이상 20이상이어야 합니다.")

    total, diaries = await diary_repository.get_list(user_id, search, sort, page, size)
...
async def update_diary(diary_id: int, user_id: int, data: DiaryUpdate):
    diary = await get_diary(diary_id, user_id)
